...
public class FlightQuery {
    ...
    /**
     * This acts as the constructor for the FlightQuery
    ...
    private boolean isValid(ArrayList<FlightInfo> path, String Origin, String Destination) {
        ...
        HashMap<String,Integer> connection = new HashMap<String,Integer>();
        try {
            Scanner in = new Scanner(new File("data/delay"));
            while (in.hasNextLine()) {
                String l = in.nextLine();
                String[] line = l.split(",");
                delay.put(line[0], Integer.parseInt(line[1]));
            }
            in = new Scanner(new File("data/connectiontime"));
            while(in.hasNextLine()){
                String l= in.nextLine();
                String[] line = l.split(",");
                connection.put(line[0],Integer.parseInt(line[1]));
            }
        }
        catch(Exception ex) {
            ex.printStackTrace();
        ...
        if (path.get(0).getOrigin().equals(Origin) && path.get(path.size()-1).getDestination().equals(Destination)) {    //First we check if origin and destination is correct
            ...
            for (int i = 1; i <= path.size() - 1; i++) {                                                                       //now we look at each flight in the path
                ...
                int Thisdelay = delay.get(f.getOrigin());
                Thisdelay = Thisdelay + connection.get(f.getOrigin());
