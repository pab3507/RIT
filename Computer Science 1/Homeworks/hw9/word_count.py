...
def printSummary( theTable ):
    ...
    maxWord = ""
    maxCount = 0
    for key in theTable.keys( ):
        thisCount = theTable.get( key )
        total += thisCount
        if thisCount > maxCount:
            maxCount = thisCount
            maxWord = key

    print( "There are " + str( len( theTable.keys( ) ) ) + " words." )
...
def printTable( hTable ):
    ...
    for key in hTable.keys( ):
        # print( "(" + key + "," + str( hTable.get( key ) ) + ")", end=" " )
        txt = "(" + key + "," + str( hTable.get( key ) ) + ")"
        ltext += len( txt )
        if ltext > 51:
            print( txt )
            ltext = 0
    ...
    while True:
        ...
        if query[0] == "k":
            print( "( " + query[1] + " in text ) is " \
                   + str( wordTable.has( query[1] ) ) + "." )

        if query[0] == "f":
            if wordTable.has( query[1] ):
